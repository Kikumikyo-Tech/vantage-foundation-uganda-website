...
function emailTemplate(title: string, rows: { label: string; value: string }[]): string {
  const tableRows = rows
    .map(
      (r) =>
        `<tr><td style="padding:4px 12px 4px 0;font-weight:bold;color:#08233a;vertical-align:top;">${r.label}</td><td style="padding:4px 0;color:#4a6072;">${r.value}</td></tr>`
    )
    .join("");
  return `<!DOCTYPE html>
<html lang="en">
...
<body style="margin:0;padding:0;background:#f4f7f9;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:24px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;border:1px solid #dce5ea;">
        <tr><td style="background:#007d8a;padding:20px 24px;">
          <h1 style="margin:0;color:#ffffff;font-size:18px;font-weight:600;">${title}</h1>
        </td></tr>
        <tr><td style="padding:24px;">
          <p style="margin:0 0 16px;color:#4a6072;">A new submission was received on the Vantage Foundation Uganda website.</p>
          <table cellpadding="0" cellspacing="0">${tableRows}</table>
          <p style="margin:24px 0 0;color:#4a6072;font-size:12px;border-top:1px solid #dce5ea;padding-top:16px;">
